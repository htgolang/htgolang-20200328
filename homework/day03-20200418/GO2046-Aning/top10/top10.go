...
func main() {
	...
	chcount := map[int][]rune{} //初始化一个map存放出现次数和字母
	for k, v := range count {
	...
	times := []int{}
	strchar := []rune{}
	for kk, _ := range chcount { //遍历次序降序
		times = append(times, kk)
	}
	//降序
	sort.Sort(sort.Reverse(sort.IntSlice(times)))
	if len(times) >= 10 { //把前10 的字符放到切片里
		for _, v := range times[:10] {
			strchar = append(strchar, chcount[v]...)
		}
	}
	for _, vv := range strchar {
		fmt.Printf("%q出现了%d次\n", vv, count[vv])
	}
	// fmt.Println(times)
	// fmt.Println(chcount)
