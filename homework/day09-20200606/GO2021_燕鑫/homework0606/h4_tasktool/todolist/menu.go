...
func TaskMain(httpaction string, httpresp *http.ResponseWriter) {
	dburl := tools.DBFILE
	user:=""
	if httpresp!=nil{
		user="root"
...
func selectAction(tsrv *TaskService, httpresp *http.ResponseWriter) {
	for {
		filterstr := ""
		if httpresp == nil {
		...
		tsrv.printLines(result, sortkey, desc, httpresp)
		if httpresp != nil {
			break
		} else {
...
func updateAction(tsrv *TaskService) {
	...
	tsrv.printLines(result, sortkey, desc,nil)
	fmt.Println(resultstr)
