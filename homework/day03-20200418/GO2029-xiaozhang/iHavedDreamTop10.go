...
const article = `I am happy to join with you today in what will go down in history as the greatest demonstration for freedom in the history of our nation.
...
func statistics() map[rune]int { //返回map值
	var countLetter = make(map[rune]int)
	//定义一个map 变量,使用for 循环所有的字符，并且进行相加
...
func insertSort(letterMap map[rune]int, isDesc bool) []rune { // 定义了一个map和排序变量，定义返回值为数组
	letterSlice := []rune{}
	// 定义了一个空切片

	for letter, _ := range letterMap {
		letterSlice = append(letterSlice, letter)
	}
	// 循环增加Slice中的数字

	for i := 1; i < len(letterSlice); i++ {
		j := i
		letterValue := letterSlice[i]
		for ; j > 0; j-- {
			if (letterMap[letterValue]-letterMap[letterSlice[j-1]] > 0) == isDesc {
				letterSlice[j] = letterSlice[j-1]
			} else {
				break
			}
		}
		letterSlice[j] = letterValue

		fmt.Println(letterSlice[j])
	}
	return letterSlice
...
func main() {
	...
	var topLetter []rune
	countLetterMap := statistics()
	sortLetterCount := insertSort(countLetterMap, true)
	if len(sortLetterCount) >= 10 {
		topLetter = sortLetterCount[:10]
	} else {
		topLetter = sortLetterCount
	}
	fmt.Println("top 10 letter")
	for _, letter := range topLetter {
		fmt.Printf("%c ---- %d\n", letter, countLetterMap[letter])
	}
}
