...
func getSclice() map[rune]int {
	// 遍历英文，获取英文单词数量进行统计出现次数
...
func main() {
	mapStr := getSclice()
	// for k, v := range sclice {
	// 	fmt.Printf("%c %v\n", k, v)
	// }
	var slice [][]int
	for k, v := range mapStr {
		slice = append(slice, []int{int(k), v})
	}
	sort.Slice(slice, func(i, j int) bool {
		return slice[i][1] > slice[j][1]
	})
	fmt.Println(slice[:10]) // 前10的字符及出现次数
}
