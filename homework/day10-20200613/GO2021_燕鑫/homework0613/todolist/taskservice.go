...
func (t *TaskService) GetByFilter(filterstr string) (tasks []*task, resultcnt int64, err error, sortby string, desc bool) {
	...
	for _, task := range t.tasks {
		...
		for i := 0; i < len(filters); i++ {
			...
			if value == "" {
				...
				return nil, 0, errors.New(errstr), "", false
			} else if value == f[1] {
				if i == len(filters)-1 {
	...
	return result, rowcount, nil, sortby, desc
}
...
func (t *TaskService) PrintLines(results []*task, sortkey string, desc bool) (theader []string, tbody []*task) {
	if sortkey == "" {
		sortkey = "id"
	}
	theader = []string{"id", "name", "starttime", "endtime", "status", "user"}
